...
class PuzzleRepository {
  ...
  Future<Puzzle?> getRandomPuzzle() async {
    final result = await _database.query('SELECT * FROM lichess_puzzle_cache ORDER BY RANDOM() LIMIT 1');
  ...
  Future<Puzzle?> getPuzzleById(String id) async {
    final result = await _database.query('SELECT * FROM lichess_puzzle_cache WHERE lichess_puzzle_id = @p0', [id]);
    if (result.isEmpty) return null;
    final row = result.first;
    return Puzzle(
      id: row['lichess_puzzle_id'] as String,
